...
void handleForm() {
  ...
  if (server.method() != HTTP_POST) {
    ...
    if (fan =="on"){
      ...
      if (co2.toInt() > 0){
        pwm = map(co2.toInt(), 400, 3500, 75, 255);
        komm.print(pwm);
        delay(timer_out.toInt());
        co2 = "0";
      }
      else{ 
      komm.print(pwm);  
