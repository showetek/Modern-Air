...
void handlePost(){
  /*
  for (fanSpeed; fanSpeed <= 250; fanSpeed = fanSpeed + schrittweite)
  {
    analogWrite(fanPin, fanSpeed);
    delay(500); 
  }

  for (fanSpeed; fanSpeed >= 75; fanSpeed = fanSpeed - schrittweite)
  {
    analogWrite(fanPin, fanSpeed);
    delay(500); 
  }
  */
  ...
  server.send(200, "text/plain", "Hope its done");
}
...
void setup(void) {
  ...
  server.on("/postform/", handlePost);            //Post-request verarbeiten
